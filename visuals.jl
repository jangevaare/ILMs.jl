function state_timeseries(event_db)
	...
	if(size(event_db.events)[2] == 4)
		...
		for t = 1:length(event_db.event_times[event_db.event_times .< Inf])
			susceptible[t] = sum(find_state(event_db, event_db.event_times[t], "S", event_db.cd))
			infectious[t] = sum(find_state(event_db, event_db.event_times[t], "I", event_db.cd))
			recovered[t] = sum(find_state(event_db, event_db.event_times[t], "R", event_db.cd))
		end
